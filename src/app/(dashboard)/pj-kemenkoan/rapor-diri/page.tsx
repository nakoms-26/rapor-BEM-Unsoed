...
export default async function PjKemenkoanRaporDiriPage() {
  ...
  const publishedById = new Map(publishedPeriods.map((period) => [period.id, period]));
  const allById = new Map(allPeriodsSorted.map((period) => [period.id, period]));
  const publishedByMonthYear = new Map(publishedPeriods.map((period) => [periodMonthYearKey(period), period]));

  const rows = (selfScores ?? [])
    .map((score) => {
      const period = resolvePublishedPeriodByScorePeriodId(score.periode_id, publishedById, allById, publishedByMonthYear);
  ...
  return (
    <section className="space-y-4">
      <div>
        <h2 className="text-2xl font-bold text-slate-900">Rapor Diri PJ Kemenkoan</h2>
        <p className="text-sm text-slate-600">Riwayat rapor pribadi untuk unit {ownedUnit?.nama_unit ?? "-"}.</p>
      </div>
