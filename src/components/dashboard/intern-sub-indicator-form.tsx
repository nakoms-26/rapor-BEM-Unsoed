...
import { saveInternSubIndicators } from "@/app/(dashboard)/pj-ppm-intern/intern-actions";
...
export function InternSubIndicatorForm({ kemenkoId, kemenkoName, periods, initialTemplates = [] }: Props) {
  ...
  async function onSubmit(values: SubIndicatorForm) {
    ...
    startTransition(async () => {
      const result = await saveInternSubIndicators({
  ...
  return (
    <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
      ...
      <Button type="submit" disabled={isPending} className="bg-teal-600 hover:bg-teal-700 text-white">
        {isPending ? "Menyimpan..." : "Simpan Sub-Indikator Internship"}
      </Button>
    </form>
  );
}
